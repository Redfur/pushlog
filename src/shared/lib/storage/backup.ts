import { getStorageAdapter, wipePushlogIndexedDatabase } from "./indexed-db-adapter";
...
const PUSHLOG_BACKUP_FORMAT = "pushlog-backup";
const PUSHLOG_BACKUP_VERSION = 1;
...
export async function restorePushlogFromBackup(backup: PushlogBackupPayload): Promise<void> {
	await wipePushlogIndexedDatabase();
	const storage = getStorageAdapter();

	await storage.setMeta(backup.meta);

	for (const row of backup.exerciseTypes) {
		await storage.putExerciseType(row);
	}
	for (const row of backup.sets) {
		await storage.putSet(row);
	}
	for (const goal of Object.values(backup.goalsByExerciseTypeId)) {
		await storage.putGoalForExercise(goal);
	}
}
