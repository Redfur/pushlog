...
- Базовый раннер: **Vitest** (`npm run test`, `npm run test:run`).
- При добавлении/изменении логики сначала покрывать тестами критичный пользовательский поток и доменные инварианты (store actions, расчёты статистики, day/timezone, валидация backup), затем вспомогательные модули.
- Для unit-тестов использовать файлы `*.test.ts` рядом с модулем.
- Любое изменение поведения в критичном домене должно сопровождаться обновлением или добавлением тестов.
